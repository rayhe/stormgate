...
/*$Id: mob_prog.c,v 1.10 2005/02/22 23:55:18 ahsile Exp $*/
		     ...
		     OBJ_DATA *obj, void *vo, CHAR_DATA *rndm)
{
 ...
 if ( !str_cmp( buf, "islegend" ) )
    {
      switch ( arg[1] )  /* arg should be "$*" so just get the letter */
        {
        case 'i': return 0;
        case 'n': if ( actor )
                     return ( IS_LEGEND( actor ) );
                  else return -1;
        case 't': if ( vict )
                     return ( IS_LEGEND( vict ) );
                  else return -1;
        case 'r': if ( rndm )
                     return ( IS_LEGEND( rndm ) );
                  else return -1;
        default:
          sprintf( log_buf, "Mob: %d bad argument to 'islegend' (%s)",
                mob->pIndexData->vnum, arg );
          bug ( log_buf, 0 );
          return -1;
        }
  ...
  /*
  if ( !str_cmp( buf, "hasunique" ) )
    {
	switch( arg[1] )
	  {
	  case 'n': if (actor)
			return actor->holdsunique;
		    else return -1;
	  default:
	    bug ( "Mob: Bad argument to 'hasunique'", 0 );
          return -1;
        }
    } */
  ...
  if ( !str_cmp( buf, "multi" ) )
    {
      switch ( arg[1] )  /* arg should be "$*" so just get the letter */
	{
	case 'i': lhsvl = mob->multied;
	          rhsvl = atoi( val );
                  return mprog_veval( lhsvl, opr, rhsvl );
	case 'n': if ( actor )
	          {
	...
	case 'r': if ( rndm )
		    ...
		    return -1;
	default:
	  bug ( "Mob: %d bad argument to 'multi'", mob->pIndexData->vnum ); 
	  return -1;
	}
    }
		   ...
		   OBJ_DATA *obj, void *vo)
{
 ...
 CHAR_DATA *vch   = NULL;
 int        count = 0;
 static int depth = 0;  /*guard against recurion  -- Manaux */
 depth++;
 if (depth > 3)
   {
     depth--;
			  ...
			  OBJ_DATA *obj, void *vo, int type )
{
  ...
  for ( mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next )
	/* mprg->status == 0 implies always active, otherwise, it's a bitvector
	&&'d with area->status that determines whether or not it's active
	*/
    if ( (mprg->type & type) && (mprg->status ? mob->pIndexData->area->status &mprg->status : 1) )
      {
			 ...
			 void *vo, int type)
{
 ...
 for ( mprg = mob->pIndexData->mobprogs; mprg != NULL; mprg = mprg->next )
   if ( ( mprg->type & type ) 
		&& (mprg->status ? mprg->status && mob->pIndexData->area->status :1)
