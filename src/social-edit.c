...
/*$Id: social-edit.c,v 1.5 2005/02/22 23:55:19 ahsile Exp $*/
...
#ifndef CONST_SOCIAL 
SOCIAL_DATA* social_table;	   /* and social table */
...
void do_sedit (CHAR_DATA *ch, char *argument)
{
	...
	if (!cmd[0])
	{
		...
		send_to_char (AT_WHITE,"  sedit new <social>\n\r", ch);
		send_to_char (AT_WHITE,"  sedit delete <social>\n\r", ch);
		send_to_char (AT_WHITE,"  sedit show <social>\n\r\n\r", ch);
		send_to_char (AT_PURPLE,"Syntax for Editing\n\r", ch);
		send_to_char (AT_WHITE,"  sedit <field> <social> <text>\n\r\n\r", ch);
		send_to_char (AT_PURPLE,"Field being one of:\n\r", ch);
		send_to_char (AT_WHITE,"  cnoarg, onoarg, cfound, ofound, vfound, cself, oself\n\r", ch);
		return;
	}
		
	if (!social[0])
	{
		send_to_char (AT_WHITE,"What social do you want to operate on?\n\r",ch);
		return;
	}
	...
	{
		send_to_char (AT_WHITE,"No such social exists.\n\r",ch);
		return;
	}
	...
	if (!str_cmp(cmd, "delete")) /* Remove a social */
	{
		...
		if (!new_table)
		{
			send_to_char (AT_WHITE,"Memory allocation failed. Brace for impact...\n\r",ch);
			return;
		}
		...
		send_to_char (AT_WHITE,"That social is history now.\n\r",ch);
	...
	else if (!str_cmp(cmd, "new")) /* Create a new social */
	{
		...
		if (iSocial != -1)
		{
			send_to_char (AT_WHITE,"A social with that name already exists\n\r",ch);
			return;
		}
		...
		send_to_char (AT_WHITE,"New social added.\n\r",ch);
	...
	else if (!str_cmp(cmd, "cnoarg")) /* Set that argument */
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"Character will now see nothing when this social is used without arguments.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	}
	...
	else if (!str_cmp(cmd, "onoarg"))
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"Others will now see nothing when this social is used without arguments.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	...
	else if (!str_cmp(cmd, "cfound"))
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"The character will now see nothing when a target is found.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	...
	else if (!str_cmp(cmd, "ofound"))
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"Others will now see nothing when a target is found.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	...
	else if (!str_cmp(cmd, "vfound"))
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"Victim will now see nothing when a target is found.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	}
	...
	else if (!str_cmp(cmd, "cself"))
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"Character will now see nothing when targetting self.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	...
	else if (!str_cmp(cmd, "oself"))
	{
		...
		if (!argument[0])
			send_to_char (AT_WHITE,"Others will now see nothing when character targets self.\n\r",ch);
		else
			act (AT_WHITE, "New message is now:\n\r$T\n\r", ch, NULL, argument, TO_CHAR );
	}
	...
	{
		send_to_char (AT_WHITE,"Huh? Try HELP SEDIT.\n\r",ch);
		return;
	}
