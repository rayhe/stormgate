...
/*$Id: mob_commands.c,v 1.5 2005/02/22 23:55:18 ahsile Exp $*/
...
void do_mpdisable(CHAR_DATA* ch, char* argument)
{
    char    	arg1[ MAX_INPUT_LENGTH ];
    char	arg2[ MAX_INPUT_LENGTH ];
    ...
    {
       bug( "Mpdisable - No argument:  vnum %d.",
	   ch->pIndexData->vnum );
       return;
    }

    if ( !( target = get_obj_here( ch, arg1 ) ) )
    {
      return;
    } else
    {
	TRAP_DATA* trap = NULL;
        ...
        for(trap = target->pIndexData->traps; trap; trap=trap->next_here)
	{
		trap->disarmed = 1;
		trap->disarm_dur = atoi(arg2);
	}
    }
...
void do_mpenable(CHAR_DATA* ch, char* argument)
{
    char    	arg1[ MAX_INPUT_LENGTH ];
    char	arg2[ MAX_INPUT_LENGTH ];
    ...
    if ( arg1[0] == '\0' || arg2[0]=='\0' )
    {
       bug( "Mpenable - No argument:  vnum %d.",
	   ch->pIndexData->vnum );
       return;
    }
